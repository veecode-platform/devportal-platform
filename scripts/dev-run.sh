...
case "$CMD" in
  ...
  run)
    ...
    fwd=""
    for v in $(compgen -e 2>/dev/null | grep -E '^(VEECODE_APP_CONFIG$|BACKSTAGE_VERSION$|NODE_ENV$|AUTH_|GITHUB_|GITLAB_|AZURE_|KEYCLOAK_|LDAP_|KONG_|SONAR|JENKINS_|K8S_|MCP_CHAT_|PLUGIN_REGISTRY$)' || true); do
