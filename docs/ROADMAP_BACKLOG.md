...
covers every dynamic plugin importing those symbols from `/alpha` — the
baked extensions module and runtime OCI plugins (e.g. immobiliarelabs
gitlab) alike. It replaced the earlier per-module `sed` patch (and the
