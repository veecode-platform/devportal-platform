...
- **RBAC plugin default-off** — the RBAC backend plugin ships `disabled: true` in the catalog; the `recommended` preset enables it. The permission *framework* (`permission.enabled: true` in `app-config.yaml`) is on regardless. See [`docs/RBAC.md`](RBAC.md).
